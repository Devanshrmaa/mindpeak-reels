...
export const neetBiologyChapters: ChapterData[] = [
  ...
  },
  {
    ...
    faqs: [
      ...
      { q: 'Which Cell Cycle & Cell Division topics get asked most in NEET PYQs?', a: 'From the last decade of papers, the recurring favourites are: the sub-stages of Prophase I (Leptotene→Zygotene→Pachytene→Diplotene→Diakinesis) and what happens in each, the DNA content (C-value) at each phase, and the differences between mitosis and meiosis. The single most-repeated fact is that crossing over happens in PACHYTENE (between non-sister chromatids, with the enzyme recombinase and a recombination nodule). Lock down Prophase I and the C-value table and you cover the bulk of the chapter\'s questions.' },
      { q: 'What is the DNA content (2C/4C) at each stage of the cell cycle?', a: 'Take G1 as the 2C baseline. DNA doubles to 4C during S phase, stays 4C through G2 and M up to metaphase. In mitosis it drops back to 2C at anaphase/telophase (sister chromatids separate to opposite poles). NEET asks this directly — e.g. "in which phase does DNA remain at 4C?" Answer: G2 and M (until anaphase). In meiosis: after Meiosis I each cell is n but still 2C (two chromatids per chromosome); only after Meiosis II is each cell n and 1C.' },
      { q: 'What chromosome number is present after Meiosis I versus Meiosis II?', a: 'This is a classic NEET trap. After Meiosis I the chromosome number is already HALVED (2n→n) because homologous chromosomes separate — but each chromosome still has two sister chromatids. Meiosis II keeps the number at n (n→n) while separating those sister chromatids. So the reduction in number happens at Meiosis I, not Meiosis II. End result: one parent cell → four haploid (n) cells.' },
      { q: 'Which are the highest-yield one-line facts to memorise for this chapter?', a: 'Synapsis (pairing of homologs) begins in Zygotene; the synaptonemal complex fully forms in Zygotene; crossing over in Pachytene; chiasmata become visible in Diplotene; terminalisation of chiasmata in Diakinesis. Interphase is the longest part of the cell cycle; Prophase is the longest stage of M phase. Cytokinesis differs in plants (cell plate, centrifugal) vs animals (cleavage furrow, centripetal). These exact one-liners recur across NEET papers — MindPeak drills them as a rapid-fire revision set.' },
    ],
  },
