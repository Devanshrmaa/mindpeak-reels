...
export const neetBiologyChapters: ChapterData[] = [
  ...
  },
  {
    ...
    topics: ['Cell Cycle Phases (G1, S, G2, M)', 'G0 (Quiescent) Phase', 'Mitosis Stages (PMAT)', 'Cytokinesis', 'Meiosis I & II', 'Prophase I Sub-stages (Leptotene to Diakinesis)', 'Crossing Over & Recombination', 'Significance of Mitosis & Meiosis', 'Differences Between Mitosis & Meiosis'],
    keyFormulas: ['Mitosis: 2n → 2n (46 → 46 in humans)', 'Meiosis: 2n → n (46 → 23 in humans)', 'Cell cycle: G1→S (DNA replication)→G2→M', 'Crossing over occurs in Pachytene (Prophase I)', 'G0 cells (no further division): neurons, cardiac muscle, mature RBCs', 'DNA doubles in S phase, chromosome number unchanged until M'],
    commonMistakes: ['Confusing stages of Prophase I (Leptotene to Diakinesis)', 'Wrong chromosome number after meiosis I vs II', 'Mixing up events of mitosis and meiosis II', 'Forgetting that crossing over produces recombinants', 'Thinking DNA content and chromosome number change together (they don\'t — S phase doubles DNA but not chromosome count)', 'Forgetting that G0 cells are metabolically active, just not dividing'],
    faqs: [
      ...
      { q: 'How many questions come from Cell Cycle & Cell Division in NEET?', a: 'Typically 3–5 questions a year, and this chapter has not had fewer than 2 in any recent NEET paper — that is 12–20 marks you can lock down from one Class 11 chapter. About 60% of those questions cluster in just three areas: Prophase I sub-stages (leptotene→pachytene→zygotene order), stage identification in mitosis (especially metaphase and anaphase), and events of the S phase. Master those three and you have covered most of what NEET repeats.' },
      { q: 'Which stage of meiosis is asked the most in NEET?', a: 'Prophase I, by a wide margin — specifically the sequence and events of its five sub-stages. Mnemonic for the order: "Lazy ZebrasPull Dirty Diapers" (Leptotene → Zygotene → Pachytene → Diplotene → Diakinesis). Crossing over happens in Pachytene; synaptonemal complex forms in Zygotene; chiasmata become visible in Diplotene. NEET loves to test "in which sub-stage does X happen", so memorise the event attached to each name, not just the order.' },
      { q: 'Which cells stay permanently in the G0 phase?', a: 'Neurons, cardiac (heart) muscle cells, and mature red blood cells exit the cycle into G0 and normally never divide again — a favourite NEET one-liner. G0 cells are not dead or inactive; they are metabolically busy, just not preparing to divide. Liver cells sit in G0 too but can re-enter the cycle if the organ is injured.' },
    ],
  },
  ...
  },
  {
    ...
    topics: ['Types of Movement', 'Skeletal Muscle Structure', 'Red vs White Muscle Fibres', 'Sliding Filament Theory', 'Mechanism of Muscle Contraction', 'Role of Ca²⁺ and ATP', 'Types of Joints (Fibrous, Cartilaginous, Synovial)', 'Axial & Appendicular Skeleton (206 bones)', 'Disorders (Myasthenia Gravis, Muscular Dystrophy, Tetany, Osteoporosis, Gout, Arthritis)'],
    keyFormulas: ['Sarcomere: Z→M→Z (A band, I band, H zone)', 'Contraction: Ca²⁺ released → Tropomyosin moves → Actin-Myosin cross bridge', 'ATP needed: for cross-bridge cycling AND Ca²⁺ pumping back', 'Adult human skeleton: 206 bones (axial 80 + appendicular 126)', 'Red fibres: more myoglobin + mitochondria, aerobic, fatigue-resistant; White fibres: less myoglobin, anaerobic, fatigue fast'],
    commonMistakes: ['Confusing A band and I band changes during contraction', 'Wrong sequence of muscle contraction events', 'Mixing up types of joints with examples', 'Forgetting that A band length doesn\'t change (actin slides)', 'Swapping the causes of the disorders (e.g. tetany = low Ca²⁺, NOT autoimmune)', 'Confusing red (slow, aerobic) and white (fast, anaerobic) muscle fibres'],
    faqs: [
      ...
      { q: 'How many questions does NEET ask from Locomotion & Movement?', a: 'Roughly 2–3 questions most years (8–12 marks). The pattern is very stable: one on the sliding-filament theory / sarcomere band changes, one factual question on a muscular or skeletal disorder, and often one on joint types or red-vs-white muscle fibres. Because the disorders are pure one-liners, this is among the cheapest chapters in Human Physiology to fully secure.' },
      { q: 'Which muscular and skeletal disorders does NEET test, and how do I keep them straight?', a: 'NEET repeats these almost every year as single-fact questions — memorise the one-line cause for each: Myasthenia Gravis = autoimmune, antibodies block acetylcholine receptors at the neuromuscular junction (fatigue/paralysis). Muscular Dystrophy = genetic, progressive degeneration of muscle. Tetany = low blood Ca²⁺ causing rapid spasms. Osteoporosis = decreased bone mass (common post-menopause, falling oestrogen). Gout = uric-acid crystal build-up in joints. Arthritis = joint inflammation. The classic trap is swapping "autoimmune" (Myasthenia Gravis) with "genetic" (Muscular Dystrophy).' },
      { q: 'What is the difference between red and white muscle fibres for NEET?', a: 'Red fibres are packed with myoglobin and mitochondria, contract slowly, run on aerobic respiration and resist fatigue (postural muscles). White fibres have little myoglobin, contract fast, rely on anaerobic respiration and tire quickly (sprinting muscles). NEET frequently asks which fibre type has more sarcoplasmic reticulum (white) or more myoglobin (red) — line these properties up in a two-column table before the exam.' },
    ],
  },
  {
