...
export const examRegistry: ExamConfig[] = [
  ...
  // ═══════════════════════════════════════════════════
  {
    ...
    faqs: [
      ...
      // mains" (23).
      { q: 'Is NSEP harder than JEE Advanced?', a: 'Yes — NSEP (National Standard Examination in Physics) is generally considered harder than JEE Advanced. It goes deeper conceptually, includes experimental-physics reasoning, and gives less time per question, so problems reward genuine understanding over speed. Compared with JEE Main, the gap is even wider. That said, JEE Advanced has a far broader syllabus and much stiffer competition for ranks, so "harder" depends on whether you mean per-question depth (NSEP) or overall selection pressure (JEE).' },
      { q: 'Is NSEC harder than JEE Advanced?', a: 'For pure question difficulty, yes — NSEC (National Standard Examination in Chemistry) asks deeper, more application-heavy questions than JEE Advanced Chemistry, especially in physical and inorganic chemistry. JEE Advanced remains broader and more competitive for admissions. Most students who prepare seriously for NSEC find JEE Advanced Chemistry more manageable afterwards.' },
