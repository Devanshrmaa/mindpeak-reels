...
/* ── Deterministic option shuffle (same as JEE page) ── */
...
const NEETPYQQuestion = () => {
  ...
  const prevSlug =
    params.questionIndex > 1
      ? `neet-pyq-${params.subject}-${params.chapter}-q${params.questionIndex - 1}`
      : null;
  const nextSlug =
  ...
  const jsonLd = [
    {
      ...
      about: { '@type': 'Thing', name: `NEET ${subj} — ${chapterName}` },
          ...
          <div className="mx-auto max-w-4xl px-4">
            ...
            <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}>
              ...
              <h1 className="font-display font-bold text-3xl sm:text-4xl text-foreground mb-3">
                {chapterName} — <span className="text-green-400">PYQ</span>
              </h1>
              <p className="text-lg text-muted-foreground">
                NEET {subj} · Previous Year Question {params.questionIndex} of {totalInChapter}
              </p>
            </motion.div>
          </div>
        </section>
        ...
        <section className="mx-auto max-w-4xl px-4 py-12 space-y-12">
          ...
          >
            <div className="flex items-start justify-between gap-4 mb-6">
              <h2 className="font-display font-bold text-xl text-foreground flex items-center gap-2">
                <BookOpen className="w-5 h-5 text-green-400 flex-shrink-0" /> Question {params.questionIndex}
              </h2>
            ...
            <div className="space-y-3 mb-6">
              {shuffledOptions.map((option, idx) => {
                ...
                if (isAnswered) {
                  ...
                  cls = 'border-green-500 bg-green-500/5';
                }
                return (
                  <button
                    key={idx}
            ...
            <div className="flex items-center justify-between pt-4 border-t border-border">
              {prevSlug ? (
                <Link to={`/${prevSlug}`} className="inline-flex items-center gap-1 text-sm font-medium text-foreground hover:text-green-400 transition-colors">
                  <ChevronLeft className="w-4 h-4" /> Prev
                </Link>
              ) : <span />}
              <span className="text-xs text-muted-foreground font-medium">Q {params.questionIndex} / {totalInChapter}</span>
              {nextSlug ? (
                <Link to={`/${nextSlug}`} className="inline-flex items-center gap-1 text-sm font-medium text-foreground hover:text-green-400 transition-colors">
          ...
          <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} className="rounded-xl border border-border bg-card/50 overflow-hidden">
            ...
            >
              <div className="flex items-center gap-2 text-sm font-semibold text-foreground">
                <List className="w-4 h-4 text-green-400" /> Choose a Different Chapter
              </div>
                      ...
                      {isExp && (
                        <div className="border-t border-border/50 px-4 py-2 space-y-1">
                          {ch.questions.slice(0, 5).map((_, qi) => (
                            <Link
                              key={qi}
                              to={`/neet-pyq-${bank.slug}-${ch.slug}-q${qi + 1}`}
                          ...
                          {count > 5 && (
                            <Link
                              to={`/neet-pyq-${bank.slug}-${ch.slug}-q1`}
          ...
          >
            <h2 className="font-display font-bold text-xl text-foreground mb-2">
              Need Help with NEET PYQ Patterns?
            </h2>
            <p className="text-muted-foreground text-sm mb-6 max-w-lg mx-auto">
              Get detailed PYQ analysis, chapter-wise weightage insights, and 1-on-1 doubt sessions with MindPeak's NEET expert mentors.
            </p>
            <button
