...
const JEEPracticeHub = () => {
  const { openDemoModal } = useDemoModal();
  const [activeSubject, setActiveSubject] = useState(subjectBanks[0].slug);
  const [expandedChapter, setExpandedChapter] = useState<string | null>(null);

  const activeBank = subjectBanks.find((b) => b.slug === activeSubject)!;
  const totalAll = subjectBanks.reduce((s, b) => s + getTotalQuestions(b.slug), 0);

  const title = 'JEE Practice Questions — Physics, Chemistry, Maths | MindPeak';
  const description = `Practice ${totalAll}+ JEE Main & Advanced MCQs across Physics, Chemistry, and Mathematics. Topic-wise, difficulty-wise questions with step-by-step solutions. Free JEE prep by MindPeak.`;

  const jsonLd = [
    {
      '@context': 'https://schema.org',
      '@type': 'CollectionPage',
      name: 'JEE Practice Questions',
      description,
    ...
    },
    {
      '@context': 'https://schema.org',
      '@type': 'ItemList',
      name: 'JEE Practice — Chapter-wise Question Banks',
      numberOfItems: subjectBanks.reduce((s, b) => s + b.chapters.length, 0),
      itemListElement: subjectBanks.flatMap((bank, si) =>
        bank.chapters.map((ch, ci) => {
          const firstTopic = ch.topics[0];
          const firstSlug = firstTopic ? getPracticeSlugByParams(bank.slug, ch.slug, firstTopic.slug, 'easy', 1) : null;
          return {
            '@type': 'ListItem',
            position: si * 100 + ci + 1,
            name: `JEE ${bank.subject} — ${ch.name}`,
            url: firstSlug ? `https://mindpeakinstitute.com/${firstSlug}` : `https://mindpeakinstitute.com/jee-practice`,
          };
        }),
      ),
    },
    ...
    <>
      ...
      <main className="bg-background pt-20">
          ...
          <div className="mx-auto max-w-6xl px-4 text-center">
            <div>
              <nav className="mb-6 text-sm text-muted-foreground flex items-center justify-center gap-1">
                ...
                <span className="text-foreground font-medium">JEE Practice Questions</span>
              </nav>
              ...
              <p className="text-lg text-muted-foreground max-w-2xl mx-auto mb-8">
                {totalAll}+ topic-wise MCQs for JEE Main & Advanced — Pick a subject, choose your chapter and start practicing.
              </p>

              {/* Subject Stats */}
              <div className="flex flex-wrap justify-center gap-4 mb-8">
                {subjectBanks.map((bank) => {
                  const count = getTotalQuestions(bank.slug);
                  return (
        ...
        <section className="mx-auto max-w-6xl px-4 py-12">
          ...
          <div className="space-y-3">
            {activeBank.chapters.map((chapter, chIdx) => {
              ...
              const qCount = getChapterQuestionCount(chapter);
        ...
        <section className="mx-auto max-w-4xl px-4 pb-12">
          ...
          >
            ...
            <p className="text-muted-foreground text-sm mb-6 max-w-lg mx-auto">
              Get personalised problem-solving sessions, doubt support, and chapter-wise tests with MindPeak's <Link to="/jee-coaching" className="text-primary font-semibold hover:underline">JEE 1-on-1 coaching program</Link>.
            </p>
            <button
        ...
        <section className="mx-auto max-w-5xl px-4 pb-16">
          <h2 className="font-display font-bold text-lg text-foreground mb-5">Explore JEE Coaching, PYQs & Free Tools</h2>
          <div className="flex flex-wrap gap-2">
            {[
              ...
              { to: '/jee-pyq', label: 'JEE Previous Year Questions (PYQs)' },
        ...
        <PageFooter extra="JEE Practice Questions — Physics, Chemistry, Mathematics." />
      </main>
    </>
  );
};

export default JEEPracticeHub;
